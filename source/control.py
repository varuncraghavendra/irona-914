...
class FlightController:
    # Position loop, as a stiff tracker for transit. The jets sit well below the
    # COM, so horizontal force and pitch are rigidly coupled: any demand to push
    # the body sideways is also a demand to pitch it. That is fine at these gains
    # while the body is a rigid lump, but see set_position_gains() for what the
    # airborne grasp needs.
    TRANSIT_GAINS=(4.0,3.3)

    def __init__(self,dt):
        self.dt=dt;self.forces=np.zeros((4,3));self.origin=None
        self.allocation_residual=np.zeros(6)
        self.target_com=None
        self.position_gain,self.damping_gain=self.TRANSIT_GAINS

    def set_position_gains(self,gains):
        """Retune the position loop, e.g. to soften it while the arm is committed.

        At the transit gains the position loop runs at about 2 rad/s against an
        attitude loop at roughly 4 rad/s. Two octaves is not enough separation for
        a plant where correcting a position error *requires* pitching first: with
        an arm swinging the COM around under it, the two loops end up chasing each
        other and the pitch wins. Slowing the outer loop down decouples them.
        """
        self.position_gain,self.damping_gain=(float(g) for g in gains)

    def update(self,t,frames,com_velocity,angular_velocity,root_R,duration=16,target=None,cut=None):
        """Allocate thruster forces to hold a COM position.

        target overrides the built-in rise-hover-descend profile with an external
        COM setpoint, which is what the airborne fetch drives. cut forces the jets
        off; with the built-in profile it defaults to the end-of-run touchdown test.
        """
        ...
        if target is None:
            ...
            if cut is None:
                cut=t>duration-.35 and com[2]<self.origin[2]+.035
        else:
            self.target_com=np.asarray(target,dtype=float)
        if cut:
            # Hold the ramp state but command nothing: the filter below then bleeds
            # the jets out over its own time constant instead of cutting a step.
            self.forces+=(1-math.exp(-self.dt/JET_TIME_CONSTANT))*(0.0-self.forces)
            self.allocation_residual=np.zeros(6)
            return self.forces.copy()
        acceleration=self.position_gain*(self.target_com-com)-self.damping_gain*np.asarray(com_velocity)
